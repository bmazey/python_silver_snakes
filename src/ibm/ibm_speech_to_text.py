...
files = ("sample_ibm_conversion_human.mp3","anakin.mp3")
for file in files:
    with open(os.path.join(os.path.dirname(__file__), "resources/" + file), 'rb') as audio_file:
        speech_recognition_results = speech_to_text.recognize(  # loads the audio file in memory
            ...
            keywords_threshold=0.3)  # using the recognize method to make a request and returns a json with translation
